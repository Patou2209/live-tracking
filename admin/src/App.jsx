...
// admin/src/App.jsx  le code ici est bon mais c'est la version sans tracer
/*import { useEffect, useRef, useState } from "react";
import maplibregl from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";
import "./App.css";
...
// admin/src/App.jsx  ceci est le code contenant le tracé
import { useEffect, useRef, useState } from "react";
import maplibregl from "maplibre-gl";
import "maplibre-gl/dist/maplibre-gl.css";
import "./App.css";
...
export default function App() {
  ...
  // --- Nouveau ---
  const pathRef = useRef([]); // pour stocker les coordonnées du trajet
  const pathSourceRef = useRef(null); // référence source GeoJSON
  ...
  // Charger les positions (liste globale)
  useEffect(() => {
    if (!user) return;
    const locRef = ref(db, "locations");
    const cb = (snap) => {
  ...
  // Suivre l'utilisateur sélectionné avec tracé
  useEffect(() => {
    if (!user) return;
    ...
    // 🔴 Réinitialiser le tracé quand on change d’utilisateur
    pathRef.current = [];
